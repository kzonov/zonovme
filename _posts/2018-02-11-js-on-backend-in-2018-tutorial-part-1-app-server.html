...
<p>app.listen(3000);</p>
<p>[/javascript]</p>
<p>I expect that you have a basic understanding of JS syntax, so the first couple of lines should be descriptive but their own. With `app.get...` block we're defining a root route. Function `get` (as well as put, post and delete) expects two arguments: query-string and callback-function. The first attribute is your HTTP route, it can be just '/' as a root route, or smth terribly-long, like 'users/:user_id/posts/:id', so it means that application server will be listening to such URL path and once called will invoke the callback function. If you're familiar with JS you may found that callback function is written using ES6. It's rather new ECMAScript standard, which is widely used now in both FE and BE JS app development. And the style we're using here is named <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions" target="_blank" rel="noopener">Arrow-function</a>. So this callback function accepts two parameters, request object and response object. From the first one you may take different input attributes, like params, headers etc. To the response object you'll send your response in (normally) JSON format. The last line says that app server should be started on the port 3000.</p>
<p>It's time to check it: <code>node app.js</code>. It won't give you any response because pure ExpressJS has nothing to tell you about itself xD But once you performed this command - go to your browser and open <code>http://localhost:3000/</code> and you'll see computer saying hello. Now you can go back to the terminal and make <code>ctrl+c</code> to kill this speechless thing.<br />
The first aspect you may want to improve is to add some kind of response to your console when you run a server and receive a request.<br />
Basically, it's two separate questions. Let's first solve the one when you want to have a message on application server start. It can be solved pretty easily. Just pass a callback to your <code>app.listen</code> function:</p>
<p>[javascript]app.listen(3000, () =&gt; console.log('Master, I started the server for you!'));<br />
[/javascript]</p>
<p>Empty brackets here needed also for Arrow function. So oldie way to write it would be:</p>
<p>[javascript]app.listen(3000, function() { console.log('Master, I started the server for you!') });<br />
[/javascript]</p>
<p>A bit extra verbose, huh? So if we have ES6 expressiveness, let's use it.<br />
The second thing we may want to add is requests logging. And this may be performed by another plugin, Morgan. To install it run <code>npm install morgan --save</code>. Then in your code add following lines:</p>
<p>[javascript]const morgan = require('morgan')<br />
app.use(morgan('dev'))<br />
[/javascript]</p>
<p>after your <code>const app = express();</code> line.</p>
<p>Now start the server. First, you'll see the welcome message right after your server successfully started. Then go to browser and request <code>http://localhost:3000/</code> again. Check the console, you'll see smth like <code>GET / 304 4.515 ms - -</code>, which says that your request returned status 304 and took 4ms to complete. Now try to request smth, we don't have yet, f.e <code>http://localhost:3000/blerg</code>, go to the console and you'll see there <code>GET /blerg 404 3.069 ms - 142</code>.</p>
<p>One thing could look unfamiliar for you so far is <code>app.use...</code> line. It means that we add middleware Morgan to our app. I'll cover this topic more thoroughly in the next NodeJS post. For now let's remember that we have linter, which must keep our code clean if we cannot and let's go to console and perform it there: <code>eslint .</code>. It will tell you that you have few errors, because I intentionally skipped some semicolons. This tool wouldn't be so widely used if it would not help us fixing all the errors (sarcasm, it CAN help you, however couldn't fix all of them). Run <code>eslint . --fix</code> and it will fix errors it found for you. You'll still have one warning about console.log statement, which is kinda ok %)</p>
<p>This is it for now. In the next post we will get more familiar with "Middleware" concept and build a basic REST API.		</p>
